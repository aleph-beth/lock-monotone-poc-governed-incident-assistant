...
\subsection{Strategic Context: Machine-Speed Conflict and the
Defender's Inherited Flaw}
For three decades, cyberdefense was calibrated on an implicit assumption:
attacks unfold slowly enough for a human analyst to observe, qualify, and
decide. The Security Operations Center (SOC) escalation cycle---alert,
triage, escalation, authorization---takes hours, at times days. Agentic-AI
campaigns compress into minutes what previously required days of specialist
work: reconnaissance, exploit generation, lateral movement, and credential
harvesting now chain at a tempo that outpaces any human-in-the-loop
response. When the attacker acts faster than the defender, the attacker
controls the tempo of the engagement---and therefore the engagement. The
structural consequence is that neither offensive (red) nor defensive (blue)
human teams will operate \emph{inside the loop}: humans remain upstream
(doctrine, authorization) and downstream (review, attribution), while the
core engagement is fought between autonomous agents---an ``AI vs.\ AI''
battlefield with no human in the
loop~\cite{sify2025aivsai, boozallen2025}.

This shift is no longer prospective. In November 2025, Anthropic disclosed
what it describes as the first large-scale cyber-espionage operation
largely orchestrated by an agentic AI: an actor assessed with high
confidence as a Chinese state group (designated GTG-1002) jailbroke an
agentic coding tool via role-play---posing as a legitimate security firm
running a defensive test---then connected it to real tools through the
Model Context Protocol. The AI executed an estimated 80--90\% of the
operation against roughly thirty targets, with a small number of successful
intrusions; notably, its principal limitation was model \emph{hallucination}
rather than any defense~\cite{anthropic2025espionage}. The trend is broader:
AI-native pentest tooling bundling thousands of exploitation prompts, an
actor exploiting CVE-2025-7775 across 8{,}000+ endpoints in under ten
minutes, ransomware chains compressed to roughly 25 minutes, and analyses
warning that AI-driven attacks already outpace human-driven defenses on
critical infrastructure~\cite{picus2025machinespeed, boozallen2025}.

The asymmetry is structural, not a recoverable lag. The natural
response---automate the defense by delegating triage, correlation, even
containment to defensive agents---closes a trap, because those defensive
agents inherit the very flaw the attacker exploits. In October 2025,
researchers from leading laboratories tested twelve published
prompt-injection defenses, most claiming near-zero attack success; the team
broke over 90\% of them~\cite{carlini2025broke}. Large-scale public
competitions on indirect injection reach the same verdict: agents remain
massively permeable~\cite{injectioncomp2026}. Automating defense with LLMs
therefore reproduces the attacker's flaw on the defender's side: an
adversary need no longer target the network directly---it can target the
\emph{supervisory agent}, which sees everything. The attack surface migrates
to the defender.

The root is architectural, not a bug to be patched by more training. An LLM
has no intrinsic boundary between \emph{instruction} and \emph{data}: the
system prompt and the user input arrive in the same form---natural-language
strings---and cannot be distinguished by type. Any separation is merely
\emph{semantic}: fuzzy, contextual, and therefore exploitable. Attention
analysis confirms it: under a successful injection, the attention of certain
heads shifts from the original instruction to the injected
one~\cite{attentiontracker2024}. An LLM excels at \emph{translating} and
\emph{classifying} but is structurally unable to maintain \emph{exact state
over a sequence}---an expressivity limit that opens \emph{accumulation
attacks}, where each turn looks benign and the violation emerges from
composition, beyond the reach of a turn-by-turn check. A direct corollary:
one LLM cannot reliably control another, since the controller shares the
controlled model's incapacity. Stacking defensive AI adds vulnerable
surfaces, not guarantees.

This is precisely the motivation for Lock-Monotone. If the model cannot be
trusted to guard itself, the security boundary must be relocated
\emph{outside} the model: confine the LLM to its strengths (translating
intent, classifying input) and delegate the \emph{decision} to a
deterministic, verifiable layer placed off the network execution path. The
prefix monotonicity developed in this paper halts accumulation attacks at a
bound that no sequence of turns can push back---because that bound is
enforced by a function, not evaluated by a model. The objective is not a
``safer'' LLM (likely out of reach) but to ensure that a compromised model
confers \emph{no authority}: the model proposes, the deterministic layer
disposes.
...
\paragraph{Compact statement} In one sentence: prompt-based security fails
because it makes the language of policy and the language of attack coincide;
Lock-Monotone normalizes intent into a finite IR, enforces policy over that
closed alphabet deterministically, and guarantees by monotonicity that no
later step reintroduces capability---so robustness to synonyms becomes a
\emph{property of normalization} rather than of the prompt author's skill,
and the decision becomes \emph{auditable by construction} (traceable,
replayable, formally verifiable).
...
\subsection{Why Prompt-Based Security Is Lexical}
Classical prompt-based security fails because it makes the \emph{language
of policy} and the \emph{language of attack} coincide: both are natural
language. When security is written into the system prompt (``never
exfiltrate'', ``never transfer data''), the rule lives in the \emph{same
lexical space} as the attack. The instruction can only ``cover'' specific
strings, and the attacker simply steps outside that coverage:
\begin{itemize}
\item \textbf{synonymy}---\emph{exfiltrate} $\rightarrow$ \emph{copy to /
relay / forward / mirror};
\item \textbf{periphrasis / obfuscation}---describing the action without
naming it;
\item \textbf{register shift}---encoding, another language, a fictional
role.
\end{itemize}
Worse, the arbiter of these rules is itself a \emph{non-deterministic LLM}:
one can neither enumerate the infinite set of reformulations nor prove the
model will refuse next time. Stated this way, security is an
\emph{unfalsifiable claim}.

Lock-Monotone breaks the coincidence by normalizing intent into the finite
IR before any decision: all synonyms of one action collapse onto a
\emph{single canonical symbol}.
\begin{verbatim}
{ "exfiltrate", "copy to", "relay",
  "forward", "mirror" }  ==>  ACTION:DATA_EXFIL
\end{verbatim}
The decision then no longer reads text; it reads a \emph{closed alphabet}.
The capability firewall is therefore deterministic, and adding a business
or security rule means adding one line over a finite vocabulary---with no
model retraining. Robustness to synonyms becomes a \emph{property of
normalization}, not of the prompt author's wording skill. The rules are
thereby decoupled from natural language: this is the mainspring of the
approach, developed formally by the typed IR (Section~\ref{sec:pyir}) and
the closed nomenclature (Section~\ref{sec:standard}).
...
\subsection{What Moves, and What Does Not Disappear}
Security is not made \emph{free}---it is \emph{relocated}. The system no
longer asks the LLM to \emph{refuse correctly} (undecidable, bypassable);
it asks the \emph{translation to be faithful}. The residual attack surface
therefore concentrates on a \emph{single point}: can an adversary force the
translator to emit a \emph{wrong or incomplete} IR---an under-translation,
a missing action, a wrong mapping? This is why the \emph{true benchmark is
NL$\rightarrow$IR fidelity}, not a block rate. Two claims must be kept
sharply distinct:
\begin{table}[h]
\centering
\begin{tabular}{@{}p{0.62\columnwidth}l@{}}
\toprule
\textbf{Claim} & \textbf{True?} \\
\midrule
``Adding rules requires zero training'' & True, and significant \\
``System security requires zero effort'' & False (effort moves to the
translation layer) \\
\bottomrule
\end{tabular}
\end{table}
It is precisely this boundary that motivates the evaluation methodology of
Section~\ref{sec:eval}: measure translation fidelity, not a refusal rate.
...
\subsection{From an Unauditable Guard to a Verifiable Property}
A natural-language guard is \emph{structurally unauditable}: one cannot
prove \emph{why} it refused, \emph{that} it will refuse, or \emph{what} it
covers---the justification lives in the weights, opaque. Because the
Lock-Monotone decision is deterministic over a finite alphabet, it is
auditable by construction along five axes:
\begin{enumerate}
\item \textbf{Traceability}---every allow/deny is the firing of a
\emph{named rule} over a \emph{named canonical symbol}: a readable log
line, \texttt{input} $\rightarrow$ \texttt{IR} $\rightarrow$
\texttt{rule R\_k} $\rightarrow$ \texttt{decision}.
\item \textbf{Explainability}---the cause lives in the language of the
policy, not in ``the model sensed it was risky''; one can cite the rule
that blocked.
\item \textbf{Determinism / replayability}---same IR yields the same
decision, reproducible for forensics and regression testing; an incident
can be replayed.
\item \textbf{Formal verifiability}---a rule set over a finite alphabet is
a \emph{decidable} object; coverage, absence of contradiction, and the
monotonicity property itself can be proven \emph{statically}, before
deployment.
\item \textbf{Separation of audit responsibilities}---the audit
\emph{factors} into two independent questions with different methods:
(a)~is the translation faithful? $\rightarrow$ an empirical
NL$\rightarrow$IR benchmark (Section~\ref{sec:eval}); (b)~is the rule set
correct? $\rightarrow$ formal verification over a finite alphabet. One no
longer audits a monolithic black box, but two separable, individually
tractable components.
\end{enumerate}
The security of an NL guard is an \emph{unfalsifiable claim}; that of
Lock-Monotone is a \emph{verifiable property}. Auditability is not a
bolted-on feature: it is the \emph{same property}---determinism over a
finite IR---viewed through the compliance lens.

\subsection{Bridge to Compliance and the SIEM}
The IR together with the decision log \emph{are} clean, structured
telemetry. They map naturally to event schemas such as OCSF and detection
formats such as Sigma. A natural-language guard produces no structured
audit trail; Lock-Monotone produces one by construction---which is what
makes it presentable to a CISO or an external auditor, and compatible with
regulatory regimes such as the EU AI Act and DORA, as well as sector
certifications for regulated data (e.g., France's PASSI and HDS). In the
SIEM example of Section~\ref{sec:siem}, this means the very decisions that
govern the analytic LLM become first-class, queryable security events.
...
campaign,'' Nov. 2025. [Online]. Available:
\url{https://www.anthropic.com/news/disrupting-AI-espionage}

\bibitem{carlini2025broke}
VentureBeat, ``12 AI defenses claimed near-zero attack success; researchers
broke all of them,'' Oct. 2025. [Online]. Available:
\url{https://venturebeat.com/security/12-ai-defenses-claimed-near-zero-attack-success-researchers-broke-all-of-them}

\bibitem{attentiontracker2024}
``Attention Tracker: Detecting Prompt Injection Attacks in LLMs,''
arXiv:2411.00348, 2024.

\bibitem{injectioncomp2026}
``How Vulnerable Are AI Agents to Indirect Prompt Injections? Insights from
a Large-Scale Public Competition,'' arXiv:2603.15714, 2026.

\bibitem{picus2025machinespeed}
Picus Security, ``What Are AI-Powered Cyberattacks? Inside Machine-Speed
Threats'' (VILLAGER; HexStrike / CVE-2025-7775), 2025. [Online]. Available:
\url{https://www.picussecurity.com/resource/blog/what-are-ai-powered-cyberattacks-inside-machine-speed-threats}

\bibitem{boozallen2025}
Industrial Cyber, ``Booz Allen warns AI-driven cyberattacks outpace
human-driven defenses across critical infrastructure,'' 2025. [Online].
Available:
\url{https://industrialcyber.co/ai/booz-allen-warns-ai-driven-cyberattacks-outpace-human-driven-defenses-across-critical-infrastructure/}

\bibitem{sify2025aivsai}
Sify, ``AI vs AI: New Cybersecurity Battlefield Where No Humans Are in the
Loop,'' 2025. [Online]. Available:
\url{https://www.sify.com/ai-analytics/ai-vs-ai-new-cybersecurity-battlefield-where-no-humans-are-in-the-loop/}
